...
- `#3f3d4a` — Indigo-slate (page background, logo box, ticker background, dark elements)
...
The logo box has: `background: #3f3d4a`, `border-radius: 4px` (no border — the dark box is self-defining on the sage header). Text and SVG fill inside the logo are `color: #e1e1e3` / `fill="#e1e1e3"`.
